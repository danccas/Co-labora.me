...
final class Doris {
  ...
  private $log = true;
  private $flog  = '/var/www/html/creainter_admin/core/logs/doris.log';
  ...
  function insert_update_plus($table , $fields , $fieldsx , $deleted = null) {
     $id = $this->insert_update($table, $fields);
     $where ="";
     foreach($fieldsx as $field => $value) {
         $where.=' '.$field.' = '.$value.' AND';
         
     }
     if($id==0){
        $id = $this->get("SELECT * FROM ".$table." WHERE ".$where." 4 = 4",true);
        $id = $id['id'];
        if($deleted == true) {
           $this->update($table, array('deleted' => null), 'id = ' . $id);
        }
     }
     return $id;
  ...
  function insert_getId($table , $fields  , $fieldsx) {
    foreach($fieldsx as $field => $value) {
      $where.=' '.$field.' = '.$value.' AND';
    }
    $id = $this->get("SELECT id FROM ".$table." WHERE ".$where." 4 = 4",true);
    //debug("SELECT id FROM ".$table." WHERE ".$where." 4 = 4");
    if(empty($id)){
